...
<!--FOTO +NOMBRE-->
<%@include file="components/profesorHeader.jsp" %>
<div class="row align-items-start" style="margin-left: 0;margin-right: 0;">
        ...
        <div class="card shadow-sm mt-5 ml-5 mr-2 mb-5">
            ...
            <div class="card-body">
                <c:forEach var="contract" items="${contracts}">
                    <a href="${pageContext.request.contextPath}/profesor/${prof.userId}/${contract.subject.id}" style="text-decoration: none;color: inherit;color: black;">
                        <div class="text-muted"><spring:message code="${contract.subject.category.toString()}"/>/<spring:message code="${contract.subject.level.toString()}"/></div>
                        <div class="border-bottom pb-3 mb-3"><c:out value="${contract.subject.name}"/></div>
                    </a>
                </c:forEach>
            </div><div class="row align-items-center" style="justify-content: center;">
            <c:if test="${pages > 1}">
                <nav>
                    <ul class="pagination" style="justify-content: center;">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li class="page-item">
                                    <a class="page-link" href="<c:url value="/profesor/${prof.userId}?page=${page-1}"/>">
                        ...
                        <c:forEach var="i" begin="1" end="${pages}">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <li class="page-item"><a class="page-link" href="<c:url value="/profesor/${prof.userId}?page=${i}"/>"><c:out value = "${i}"/></a></li>
                                </c:otherwise>
                            </c:choose>
    ...
    <div class="col-8">
        <div class="card shadow-sm mt-5 mr-5 mb-5">
                        ...
                        <a draggable="false" class="nav-item nav-link" id="nav-review-tab" data-toggle="tab"
                           ...
                           role="tab" aria-controls="nav-review" aria-selected="false">
                            <spring:message code="clases.reviews"/>
                             <span>(<c:out value="${totalReviews}"/>)</span>
                        </a>
                    </div>
                </nav>
            ...
            <div class="card-body">
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-info" role="tabpanel"
                        ...
                        <div class="border-bottom pb-3 mb-3"><c:out value="${prof.description}"/></div>
                        ...
                        <div class="text-muted"><spring:message code="form.schedule"/></div>
                        <div class="border-bottom pb-3 mb-3"><c:out value="${prof.schedule}"/></div>

                        <div class="text-muted"><spring:message code="materia.localidad"/></div>
                        <div class="border-bottom pb-3 mb-3"><c:out value="${prof.location}"/></div>
                    </div>
                                ...
                                <div class="row" style="display: flex; justify-content: space-between;">
                                    ...
                                    <div class="col align-self-center">
                                        <div>
                                            ...
                                            <a href="${pageContext.request.contextPath}/profesor/${prof.userId}/${review.lesson.subject.id}"
                                               style="color: black;">
                                                <h5 class="font-italic mb-0" style="font-size: medium;"><spring:message code="email.materia" arguments="${review.lesson.subject.name}"/></h5>
                                            </a>
                                        </div>
                                    </div>
                                ...
                                <nav>
                                    <ul class="pagination" style="justify-content: center;">
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <li class="page-item">
                                                    <a class="page-link" href="<c:url value="/profesor/${prof.userId}?rPage=${rPage-1}"/>">
                                                        <spring:message code="pag.anterior" /></a>
                                                </li>
                                            </c:otherwise>
                                        </c:choose>
                                        ...
                                        <c:forEach var="i" begin="1" end="${rPages}">
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <li class="page-item"><a class="page-link" href="<c:url value="/profesor/${prof.userId}?rPage=${i}"/>"><c:out value = "${i}"/></a></li>
                                                </c:otherwise>
                                            </c:choose>
            ...
            <c:if test="${isCurrentUser}"> <div class="card-footer" style="text-align-last: right;">
            <a href="<c:url value="/editarProfessor" />"
               class="btn text-white" style="background-color: #349AC2;">
                <spring:message code="professor.editarPerfil" /></a>
            </div>
            </c:if>
        </div>
